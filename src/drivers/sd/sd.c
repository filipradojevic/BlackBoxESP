...
uint32_t sd_init(sd_t* sd, spi_device_handle_t handle, gpio_num_t cs)
{
	...
	printf("Initialize CS pin...\n");
	gpio_reset_pin(cs);
	gpio_set_direction(cs, GPIO_MODE_OUTPUT);
	gpio_set_level(cs, 1); // deassert
	printf("CS pin number: %d\n", cs);
	...
	if (retry >= SD_IDLE_RETRY_CNT){
		printf("SD init: CMD0 failed, retries=%d\n", retry);
		return 1;
	}
	...
	if (retry >= SD_IDLE_RETRY_CNT){
		printf("SD init: CMD8 failed, retries=%d\n", retry);
		return 1;
	}

	printf("SD init: CMD8 OK (check_pattern=0x%02X, voltage_accepted=0x%02X)\n", r7.val[3], r7.val[2]);
	...
	if (retry >= SD_IDLE_RETRY_CNT) {
		printf("SD init: CMD58 second read OK (OCR=0x%08lX)\n", (unsigned long)sd->ocr);
		return 1;
	}
	...
	if (retry >= SD_IDLE_RETRY_CNT) {
		printf("SD init: ACMD41 failed, retries=%d\n", retry);
		return 1;
	}
	printf("SD init: ACMD41 OK\n");
	...
	if (retry >= SD_IDLE_RETRY_CNT) {
		printf("SD init: CMD58 second read failed, retries=%d\n", retry);
		return 1;
	}
	printf("SD init: CMD58 second read OK");
	...
	if (retry >= SD_IDLE_RETRY_CNT) {
		printf("SD init: CMD9 failed, retries=%d\n", retry);
		return 1;
	}
	printf("SD init: CMD9 OK\n");
