 ...
 *
 * The helpers are designed to keep the FTP-related code modular and
 * maintainable, avoiding bloated monolithic files by separating common FTP
 * functionalities.
 *
...
void ftp_save_last_response(mavlink_file_transfer_protocol_t* data,
							mavlink_file_transfer_protocol_t* last_response)
{
	memcpy(last_response, data, sizeof(mavlink_file_transfer_protocol_t));
}

void ftp_timeout(QueueHandle_t queue, mav_t* mav_gw_sky_handle,
				 mavlink_message_t* tx_msg, uint8_t time2try, uint8_t delay)
{
...
int ftp_allocate_session(session_t* sessions, char* filepath)
{
	...
	for (int i = 0; i < FTP_MAX_SESSIONS; i++) {
		...
		if (!sessions[i].in_use) {
			...
			/* Check if it's available */
			if (filepath) {
				strncpy(sessions[i].filepath, filepath, FTP_FILE_NAME_SIZE - 1);
				sessions[i].filepath[FTP_FILE_NAME_SIZE - 1] = '\0';
			} else {
...
int ftp_remove_recursive(lfs_t* lfs, char* path)
{
	...
	if (info.type == LFS_TYPE_DIR) {
		...
		while (true) {
			...
			if (err <= 0)
				break; /* No more entries or error occurred */
			...
			snprintf(fullpath, sizeof(fullpath), "%.*s/%.*s",
					 (int)(sizeof(fullpath) / 2 - 1), path,
					 (int)(sizeof(fullpath) / 2 - 1), info.name);
...
uint32_t ftp_calculate_crc32_for_file(lfs_t* lfs, const char* path,
									  ...
									  int* err_code)
{
	...
	*err_code = lfs_file_opencfg(lfs, &file, path, LFS_O_RDONLY, cfg);
	...
	// Read file in chunks and update CRC
	while ((bytes_read = lfs_file_read(lfs, &file, buffer, sizeof(buffer))) >
		   0) {
	...
	*err_code = 0;
	...
	// MAVLink does not use final XOR (~crc), just return raw CRC
	return crc;
}