...
/* Flash */
flash_t flash_dev __attribute__((section("AHBSRAM0")));

#else

/* sd card */
sd_t sd __attribute__((section("AHBSRAM0")));
...
void app_main(void)
{
	...
	/* Queues motor */
	queue_ulog_motor_scaled =
		xQueueCreate(20, sizeof(ulog_lisum_power_motor_scaled_data_t));
	...
	queue_ulog_scaled_pressure =
		xQueueCreate(10, sizeof(ulog_scaled_pressure_t));
	...
	/* Queues lisum */
	queue_ulog_lisum_gnss_data1 =
		xQueueCreate(1, sizeof(ulog_lisum_gnss_recv_data_t));
	queue_ulog_lisum_gnss_data2 =
		xQueueCreate(1, sizeof(ulog_lisum_gnss_recv_data_t));
	queue_ulog_lisum_gnss_data3 =
		xQueueCreate(1, sizeof(ulog_lisum_gnss_recv_data_t));
	queue_ulog_lisum_airspeed_data =
		xQueueCreate(1, sizeof(ulog_lisum_sensor_airspeed_data_t));
	queue_ulog_lisum_act_data =
		xQueueCreate(10, sizeof(ulog_lisum_power_hornet_act_data_t));
	queue_ulog_lisum_manual_ctrl =
		xQueueCreate(1, sizeof(ulog_lisum_manual_ctrl_hornet_t));

	/* Queues ftp */
	queue_mav_ftp_command =
		xQueueCreate(1, sizeof(mavlink_file_transfer_protocol_t));

	/* Queues test */
	queue_ulog_sinusoid_test =
		xQueueCreate(15, sizeof(ulog_named_value_float_t));
	...
	ESP_ERROR_CHECK(
		spi_bus_initialize(FLASH_SPI_HOST, &flash_buscfg, SPI_DMA_CH_AUTO));
	...
	ESP_ERROR_CHECK(
		spi_bus_add_device(FLASH_SPI_HOST, &flash_devcfg, &flash_spi_handle));
	...
	do {
		status = FLASH_Init(&flash_dev, flash_spi_handle, FLASH_CS_PIN,
							HAL_GetTimeUS);
	...
	ESP_ERROR_CHECK(
		spi_bus_initialize(SD_SPI_HOST, &sd_buscfg, SPI_DMA_CH_AUTO));

	spi_device_interface_config_t sd_devcfg = {
		.clock_speed_hz = SD_SPI_INIT_CLK,
		.mode = 0,
	...
	ESP_ERROR_CHECK(
		spi_bus_add_device(SD_SPI_HOST, &sd_devcfg, &sd_spi_handle));
	...
	do {
		int32_t ret = sd_init(&sd, (void*)SD_SPI_HOST, (void*)sd_spi_handle);
	...
	result = xTaskCreate(task_work, "work", 512, NULL, 1, &task_work_handle);
	if (result != pdPASS) {
		for (;;)
			;
	}

	result = xTaskCreate(task_log, "log", 1024, NULL, 1, &task_log_global);
	if (result != pdPASS) {
		for (;;)
			;
	}
